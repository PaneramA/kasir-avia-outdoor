...
const AdminRegistrations = () => {
  ...
  return (
    <div className="space-y-5">
      {message && <div className="rounded-md border border-[#acd9c8] bg-[#edf9f4] p-3 text-sm text-[#176456]">{message}</div>}
      {(errorMessage || queryErrorMessage) && <div className="rounded-md border border-[#e9b7b7] bg-[#fff1f1] p-3 text-sm text-[#a82f2f]">{errorMessage || queryErrorMessage}</div>}

      <section className="rounded-md border border-[#dce3e6] bg-white p-4">
        <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_180px_auto_auto]">
          <div className="relative">
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Cari nama, slug, atau owner" className="min-h-11 w-full rounded-md border border-[#cfd8dc] pl-10 pr-3 text-sm outline-none focus:border-[#2a7c6f]" />
          </div>
          <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="min-h-11 rounded-md border border-[#cfd8dc] bg-white px-3 text-sm outline-none focus:border-[#2a7c6f]">
            <option value="all">Semua status</option>
            <option value="active">Aktif</option>
            <option value="suspended">Suspended</option>
          </select>
          <button type="button" onClick={() => void handleRefresh()} disabled={tenantQuery.isValidating || planQuery.isValidating} className="min-h-11 rounded-md border border-[#cfd8dc] px-4 text-sm font-semibold text-[#52616b] hover:border-[#2a7c6f] disabled:opacity-60"><i className={`fas fa-rotate-right mr-2 ${(tenantQuery.isValidating || planQuery.isValidating) ? 'animate-spin' : ''}`} />Perbarui</button>
          <button type="button" onClick={() => setIsOnboardingOpen(true)} className="min-h-11 rounded-md bg-[#173f3a] px-4 text-sm font-bold text-white hover:bg-[#0f302c]"><i className="fas fa-plus mr-2" />Tambah toko</button>
        </div>
      </section>

      <div className="grid min-h-[620px] gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
        <section className="overflow-hidden rounded-md border border-[#dce3e6] bg-white">
          ...
          <div className="max-h-[680px] overflow-y-auto p-2">
            ...
            {!isLoading && filteredTenants.map((tenant) => (
              <button
                ...
                className={`mb-1 w-full rounded-md border p-3 text-left transition ${selectedTenantId === tenant.id ? 'border-[#76a69d] bg-[#eef7f5]' : 'border-transparent hover:bg-[#f4f6f7]'}`}
              >
        ...
        <section className="rounded-md border border-[#dce3e6] bg-white">
            ...
            <form onSubmit={handleSave}>
              <div className="flex flex-col gap-3 border-b border-[#e6ebed] p-5 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex flex-wrap gap-2">
                  <button type="button" onClick={openDeleteDialog} className="min-h-11 rounded-md border border-[#e1aaaa] bg-white px-4 text-sm font-bold text-[#b52f2f] hover:bg-[#fff1f1]"><i className="fas fa-trash-can mr-2" />Hapus toko</button>
                  <button type="submit" disabled={savingTenantId === selectedTenant.id} className="min-h-11 rounded-md bg-[#173f3a] px-5 text-sm font-bold text-white hover:bg-[#0f302c] disabled:opacity-60"><i className="fas fa-floppy-disk mr-2" />{savingTenantId === selectedTenant.id ? 'Menyimpan...' : 'Simpan perubahan'}</button>
                </div>
              </div>
              ...
              <div className="grid gap-6 p-5 lg:grid-cols-2 lg:p-6">
                <div className="space-y-4">
                  ...
                  <div><label className="mb-1.5 block text-xs font-semibold text-[#52616b]">Nama toko</label><input value={selectedDraft.name} onChange={(event) => updateDraft({ name: event.target.value })} minLength="2" maxLength="120" className="min-h-11 w-full rounded-md border border-[#cfd8dc] px-3 text-sm outline-none focus:border-[#2a7c6f]" required /></div>
                  <div><label className="mb-1.5 block text-xs font-semibold text-[#52616b]">Slug</label><input value={selectedDraft.slug} onChange={(event) => updateDraft({ slug: event.target.value })} minLength="2" maxLength="80" className="min-h-11 w-full rounded-md border border-[#cfd8dc] px-3 text-sm outline-none focus:border-[#2a7c6f]" required /></div>
                  <div><label className="mb-1.5 block text-xs font-semibold text-[#52616b]">Status toko</label><select value={selectedDraft.tenantStatus} onChange={(event) => updateDraft({ tenantStatus: event.target.value })} className="min-h-11 w-full rounded-md border border-[#cfd8dc] bg-white px-3 text-sm outline-none focus:border-[#2a7c6f]"><option value="active">Aktif</option><option value="suspended">Suspended</option></select></div>

                  <h3 className="border-b border-[#edf0f1] pb-2 pt-2 text-sm font-bold">Masa langganan</h3>
                  <div className="grid grid-cols-2 gap-3">
                    <div><label className="mb-1.5 block text-xs font-semibold text-[#52616b]">Mulai</label><input type="date" value={selectedDraft.startsAt} onChange={(event) => updateDraft({ startsAt: event.target.value })} className="min-h-11 w-full rounded-md border border-[#cfd8dc] px-3 text-sm" /></div>
                    <div><label className="mb-1.5 block text-xs font-semibold text-[#52616b]">Berakhir</label><input type="date" value={selectedDraft.endsAt} onChange={(event) => updateDraft({ endsAt: event.target.value })} className="min-h-11 w-full rounded-md border border-[#cfd8dc] px-3 text-sm" /></div>
                  </div>
                  <div><label className="mb-1.5 block text-xs font-semibold text-[#52616b]">Batas grace period</label><input type="date" value={selectedDraft.graceEndsAt} onChange={(event) => updateDraft({ graceEndsAt: event.target.value })} className="min-h-11 w-full rounded-md border border-[#cfd8dc] px-3 text-sm" /></div>
                </div>

                <div className="space-y-4">
                  <h3 className="border-b border-[#edf0f1] pb-2 text-sm font-bold">Paket & akses</h3>
                  <div><label className="mb-1.5 block text-xs font-semibold text-[#52616b]">Paket langganan</label><select value={selectedDraft.planId} onChange={(event) => updateDraft({ planId: event.target.value })} className="min-h-11 w-full rounded-md border border-[#cfd8dc] bg-white px-3 text-sm outline-none focus:border-[#2a7c6f]" required>{plans.map((plan) => <option key={plan.id} value={plan.id}>{plan.name} · {formatCurrency(plan.priceAmount)}</option>)}</select></div>
                  <div><label className="mb-1.5 block text-xs font-semibold text-[#52616b]">Status subscription</label><select value={selectedDraft.subscriptionStatus} onChange={(event) => updateDraft({ subscriptionStatus: event.target.value })} className="min-h-11 w-full rounded-md border border-[#cfd8dc] bg-white px-3 text-sm outline-none focus:border-[#2a7c6f]"><option value="trial">Trial</option><option value="active">Aktif</option><option value="suspended">Suspended</option><option value="expired">Expired</option></select></div>

                  {selectedPlan && (
                    <div className="rounded-md border border-[#d8e5e2] bg-[#f3f8f7] p-4">
                  ...
                  <div><label className="mb-1.5 block text-xs font-semibold text-[#52616b]">Catatan billing</label><textarea value={selectedDraft.billingNotes} onChange={(event) => updateDraft({ billingNotes: event.target.value })} maxLength="300" rows="5" className="w-full rounded-md border border-[#cfd8dc] p-3 text-sm outline-none focus:border-[#2a7c6f]" placeholder="Catatan pembayaran atau onboarding" /><p className="mt-1 text-right text-xs text-[#85919a]">{selectedDraft.billingNotes.length}/300</p></div>
                </div>
              </div>
            </form>
          )}
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#17202a]/55 p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) closeDeleteDialog() }}>
          <section role="dialog" aria-modal="true" aria-labelledby="delete-tenant-title" className="w-full max-w-[500px] rounded-md border border-[#dce3e6] bg-white shadow-none">
            <div className="flex items-start justify-between gap-4 border-b border-[#e6ebed] p-5">
              <div className="flex gap-3">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md bg-[#fff0f0] text-[#b52f2f]"><i className="fas fa-triangle-exclamation" /></span>
                <div>
              ...
              <button type="button" aria-label="Tutup dialog" onClick={closeDeleteDialog} disabled={isDeleting} className="flex h-9 w-9 items-center justify-center rounded-md text-[#71808a] hover:bg-[#f1f3f4]"><i className="fas fa-xmark" /></button>
            </div>

            <form onSubmit={handleDelete} className="space-y-4 p-5">
              <div className="rounded-md border border-[#e9b7b7] bg-[#fff5f5] p-3 text-sm leading-relaxed text-[#8f2d2d]">
              ...
              <div>
                ...
                <div className="mb-2 rounded-md bg-[#f3f5f6] px-3 py-2 font-mono text-sm font-bold text-[#17202a]">{deleteTarget.name}</div>
                <input
                  ...
                  autoComplete="off"
                  className="min-h-11 w-full rounded-md border border-[#cfd8dc] px-3 text-sm outline-none focus:border-[#b52f2f] focus:ring-2 focus:ring-[#b52f2f]/10"
                  required
                  autoFocus
                />
              </div>
              ...
              <div className="flex flex-col-reverse gap-2 pt-2 sm:flex-row sm:justify-end">
                <button type="button" onClick={closeDeleteDialog} disabled={isDeleting} className="min-h-11 rounded-md border border-[#cfd8dc] px-4 text-sm font-semibold text-[#52616b] hover:bg-[#f4f6f7] disabled:opacity-60">Batal</button>
                <button
                  type="submit"
                  disabled={isDeleting || deleteForm.confirmationText !== deleteTarget.name || !deleteForm.password}
                  className="min-h-11 rounded-md bg-[#b52f2f] px-5 text-sm font-bold text-white hover:bg-[#912525] disabled:cursor-not-allowed disabled:opacity-45"
                >
