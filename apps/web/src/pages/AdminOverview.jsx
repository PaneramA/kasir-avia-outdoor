...
const AdminOverview = ({ currentUser }) => {
  ...
  return (
    <div className="space-y-5">
      <section className="flex flex-col gap-4 rounded-md border border-border bg-card-bg p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
        <div>
          <p className="text-sm text-text-muted">Selamat datang, {currentUser?.username || 'Admin'}</p>
          <h2 className="mt-1 text-xl font-bold sm:text-2xl">Ringkasan platform</h2>
        </div>
        <button type="button" onClick={() => void refreshData()} disabled={isRefreshing} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-md border border-border px-4 text-sm font-semibold text-text-muted hover:border-accent disabled:opacity-60">
          <i className={`fas fa-rotate-right ${isRefreshing ? 'animate-spin' : ''}`} />
          Perbarui data
        </button>
      </section>

      {errorMessage && <div className="rounded-md border border-[#e9b7b7] bg-[#fff1f1] p-3 text-sm text-[#a82f2f]">{errorMessage}</div>}

      <section className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {stats.map((stat) => (
          <article key={stat.label} className="rounded-md border border-border bg-card-bg p-4 sm:p-5">
            <span className={`flex h-9 w-9 items-center justify-center rounded-md ${stat.tone}`}><i className={stat.icon} /></span>
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1.5fr)_minmax(280px,0.7fr)]">
        <div className="overflow-hidden rounded-md border border-border bg-card-bg">
          <div className="flex items-center justify-between border-b border-border px-5 py-4">
            <div>
              ...
              <p className="mt-0.5 text-xs text-text-muted">Status dan paket saat ini</p>
            </div>
            <Link to={APP_ROUTES.adminStores} className="text-sm font-semibold text-accent hover:underline">Lihat semua</Link>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[640px] border-collapse text-left text-sm">
              <thead className="bg-bg-main text-xs uppercase text-text-muted">
                <tr>
              ...
              <tbody className="divide-y divide-border">
                {recentTenants.map((tenant) => (
                  <tr key={tenant.id}>
                    <td className="px-5 py-3.5"><p className="font-semibold">{tenant.name}</p><p className="text-xs text-text-muted">{tenant.slug}</p></td>
                    <td className="px-5 py-3.5 text-text-muted">{tenant.subscription?.plan?.name || '-'}</td>
                    ...
                    <td className="px-5 py-3.5 text-text-muted">{formatDate(tenant.createdAt)}</td>
                  </tr>
                ))}
                {!isLoading && recentTenants.length === 0 && <tr><td colSpan="4" className="px-5 py-8 text-center text-text-muted">Belum ada toko.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div className="space-y-3">
          <Link to={APP_ROUTES.adminStores} className="flex items-center justify-between rounded-md border border-border bg-card-bg p-4 transition hover:border-accent">
            <span className="flex items-center gap-3"><i className="fas fa-store text-accent" /><span className="text-sm font-semibold">Kelola toko</span></span><i className="fas fa-chevron-right text-xs text-text-muted" />
          </Link>
          <Link to={APP_ROUTES.adminPlans} className="flex items-center justify-between rounded-md border border-border bg-card-bg p-4 transition hover:border-accent">
            <span className="flex items-center gap-3"><i className="fas fa-sliders text-[#465ca8]" /><span className="text-sm font-semibold">Edit paket & fitur</span></span><i className="fas fa-chevron-right text-xs text-text-muted" />
          </Link>
          {pendingTenants.length > 0 && (
            <div className="rounded-md border border-[#e4c97b] bg-[#fff8df] p-4">
